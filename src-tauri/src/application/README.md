...
- ✏️ **Commands**: Actions that **mutate** state (create, delete, restore snapshots)
- 🔍 **Queries**: Actions that **read** state without modifying it

```mermaid
graph LR
    subgraph "✏️ Commands (Write)"
        CS["📸 CreateSnapshot"]
        DS["🗑️ DeleteSnapshot"]
        RS["🔄 RestoreSnapshot"]
    end
...
| `DeleteSnapshotHandler` | `DeleteSnapshotCommand { snapshot_id }` | 🗑️ Remove from DB | `snapshot.delete` |
